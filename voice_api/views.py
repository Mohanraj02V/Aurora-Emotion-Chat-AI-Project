...
def process_text_message(request):
    ...
    try:
        ...
        # Process message with emotion detection
        result = voice_ai_service.process_user_message(text, user, conversation_history)
...
def process_audio_message(request):
    ...
    try:
        ...
        temp_filename = f"recording_{uuid.uuid4().hex}{file_extension}"
        ...
        # If transcription failed with error, return early
        if any(keyword in transcription.lower() for keyword in ['error', 'unavailable', 'not found', 'no audio']):
            return Response({
